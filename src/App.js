  ...
  useEffect(function () {
    async function fetchMovies() {
      try {
        ...
        if (!res.ok) throw new Error("quelque chose ne marche pas avec le fecth movies")
      ...
      {isloading ? (<Loading />) : (
        <>
          <header>
            ...
            <img src={poster} alt={`Poster of ${film} movie`} />
...
function WathchedMovie({ movie, onDeleteWatched }) {
  return (
    <li >
      <img src={movie.poster} alt={`${movie.title} poster`} />
      <h3>{movie.title}</h3>
